...
struct CleanZeroWidthPass : public Pass {
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto module : design->selected_modules())
		{
			...
			// For processes, do the bare minimum: remove any memory writes
			// to 0-width memories, and widen 0-width addresses.
			for (auto &it: module->processes) {
				if (!design->selected(module, it.second))
					continue;
				for (auto sync : it.second->syncs) {
					std::vector<int> swizzle;
					std::vector<RTLIL::MemWriteAction> new_actions;
					for (int i = 0; i < GetSize(sync->mem_write_actions); i++) {
						...
						swizzle.push_back(i);
						new_actions.push_back(memwr);
					}
					std::swap(new_actions, sync->mem_write_actions);
				}
			}
